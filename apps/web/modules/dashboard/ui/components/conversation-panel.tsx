...
import { ListIcon, ArrowLeftIcon, ArrowUpIcon, CheckIcon, CornerUpLeftIcon, ArrowRightIcon } from "lucide-react"
...
export const ConversationPanel =() =>{
    ...
    return (
        <div className="flex h-full w-full flex-col bg-background text-sidebar-foreground">
            ...
            {
                isLoadingFirstPage ? <SkeletonConversations /> :
            
            <ScrollArea className="max-h-[calc(100vh-53px)]">
             <div className="flex w-full flex-1 flex-col text-sm">
                { conversations.results.map((conversations=>{
                    const isLastMessageFromOperator = 
                    conversations.lastMessage?.message?.role !== "user";
                    ...
                    return (
                        ...
                        className={cn("relative flex cursor-pointer items-start gap-3 border-b p-4 py-5 text-sm leading-tight hover:bg-accent hover:text-accent-foreground",
                            pathname === `/conversations/${conversations._id}` && 
                            "bg-accent text-accent-foreground"
                        )}
                        href={`/conversations/${conversations._id}`}>
                            <div className={cn("-translate-y-1/2 absolute top-1/2 left-0 h-[64%] w-1 rounded-r-full bg-neutral-300 opacity-0 transition-opacitiy",
                                pathname === `conversations/${conversations._id}` && "opacity-100"
                            )} />


                            <DicebearAvatar
                            seed={conversations.contactSession._id}
